...
5. 新增特性 (New Features V3.1)

5.1 智能引导与卡片筛选 (Smart Onboarding)
*   **流程**: 注册 -> 问卷 (身份/目标/水平) -> AI 生成 (10张) -> **卡片筛选 (Selection)** -> Dashboard。
*   **筛选逻辑**: 用户可预览生成的 10 张卡片，勾选不认识的单词加入学习，剔除已掌握的单词。

5.2 划词闪充 (Quick Add)
*   **场景**: 在学习界面的例句 (Example/Shadow) 中，遇到不认识的单词。
*   **交互**: 鼠标选中单词 -> 弹出 "Add [Word]" 气泡 -> 点击即刻生成新卡片并存入词库。

5.3 双语支持 (Bilingual UI)
*   支持中/英界面切换。
*   AI 生成内容根据用户偏好自动适配翻译语言 (默认中文翻译)。